...
7. **Async cloud odpověď ulož jen do okna, pro které se stahovala** (jen u app s víc okny dne / per-okno `WK()` namespace). `silentPull()` i `checkCloudOnStartup()` si vezmou `gistId` synchronně, ale `await gistGet()` trvá — když mezitím uživatel překlikne okno, `autoImportSilent()` by sloučil stažená data do globálů (`library`/`weekDays`/…) a `saveLibrary()`/`saveWeek()` je uloží do `WK()` **aktuálního** okna → data jednoho okna přepíšou druhé. Zachyť `const winAtStart = activeWin;` na začátku a před aplikací dej `if (activeWin !== winAtStart) return;`. Registr oken (`mergeWindowRegistry`) je globální → ten slučuj **před** kontrolou. Push (`doAutoPush`/`doCloudUpload`) řešit netřeba — `gistId` i `payload` bere synchronně pohromadě, takže je vždy konzistentní. *(Výjimka: `doAutoPush` dnes umí před pushem await-nout dopull — pak si po awaitu musí `activeWin !== winAtStart` zkontrolovat taky, viz past 8.)*
8. **Read-before-overwrite: nikdy nepushuj na základě starého čtení.** PATCH přepisuje celý soubor gistu (GitHub neslučuje na serveru) → payload postavený nad zastaralým stavem tiše smaže vše, co mezitím nahrála jiná zařízení. Dvě vrstvy: (a) **brána pull-before-push** (`initialPullDone`) se otevře **jen po úspěšném čtení cloudu** — selhaný pull (offline, 403 rate limit, 5xx, nečitelný/truncated obsah, mezitím přepnuté okno) musí bránu nechat ZAVŘENOU a smazat session flag `…cloud-checked::<win>`, aby to příští focus/interval pull zkusil znovu; `404` je výjimka (gist smazaný → není co chránit, push ho založí znovu). (b) **stáří čtení** — `lastPullOkTs` per okno; když je poslední úspěšné čtení starší než `PUSH_PULL_MAX_AGE` (2 min), `doAutoPush` si nejdřív `await silentPull(true)` (dopull + merge) a teprve pak staví payload; když ani ten pull neprojde, push se odloží (`pushDeferred`). Bez (b) appka nechaná přes den v pozadí (nic nepulluje, dokud není vidět) při první editaci přepíše gist ranním snapshotem.
9. **Registr oken je jen v hlavním gistu → ve vedlejším okně ho dopullni zvlášť** (`ensureRegistryFresh`). Zařízení, které nastartuje rovnou ve vedlejším okně, jinak nikdy neuvidí registr; pokud mu chybí Gist ID toho okna, `checkCloudOnStartup` spadne do větve „okno nemá gist" → **založí NOVÝ gist** z lokálních (starých) dat a orazítkuje do registru novější `binIdMt` → ostatní zařízení se přepnou na něj a skutečný gist okna (s daty z druhého PC) osiří = „data zmizela". Než prohlásíš, že okno gist nemá, přečti registr z `todo-app-gist-id` (hlavní gist) a zkus ID dohledat.
10. **Při přepnutí okna nereconcileuj nad rozpracovaným stavem.** `activateWindow()` přebírá tombstones/baseline nového okna hned na začátku, ale kolekce (`library`, `weekDays`, `evals`, …) vyměňuje postupně — mezitím jsou globály MIX obou oken. Jakýkoliv save v té mezeře (typicky migrace v loaderu) by porovnal baseline nového okna s daty starého a **otombstonoval vše, co v tom druhém okně není** (`wd:`/`wp:`/`ev:`/`inb:` klíče mají stejný tvar napříč okny) → push to smaže na všech zařízeních. Drž po dobu výměny příznak (`windowLoading`), který hook `autoSyncOnSave()` vyřadí, a po dokončení loadů zavolej reconcile jednou explicitně.
...
**Stav:** základ je v `_template/index.html` (LWW pro `library`, append-only pro `archive`, tombstones, `lwwMerge`, `autoImportSilent`, `sigOf`, `☁` tlačítko, bezpečný bootstrap pull→merge→push, 🔗 sync odkaz pro nové zařízení, **forward-compat guard** `APP_SCHEMA` + `checkCloudSchema` — viz výše). Rozšiřovací body jsou označené `// APP LOGIC` — při přidání kolekce uprav: `currentKeySet()`, `buildAutoPayload()`, `autoImportSilent()`, `sigOf()`, `applyTombstones()` (jen append-only), seznam obalených save-funkcí a u object-map (bloky) přidej vlastní `mergeBlocksLww`. **Plný příklad se všemi typy kolekcí:** `todo-app/index.html` (dny, plány, šablony, inbox = LWW; bloky = LWW object-mapa; archiv = union). Ochrany z pastí 8–10 (brána jen po úspěšném pullu, dopull při zastaralém čtení, registr pro vedlejší okna, `windowLoading`) jsou zatím **jen v `todo-app`** — do `_template` portovat na pokyn.
